...
def upload_document(api_key, file_storage, workflow_id, mode=None):
    from services import dify_service

    """Upload file to Dify using api_key, do not save locally. Accepts mode (CLOUD/LOCAL)."""
    ...
    try:
        ...
        if mode is None:
            mode = dify_service.DifyMode.CLOUD
        dify_document_id = dify_service.upload_document_to_dify(
            api_key, filepath, filename, file_storage.mimetype, "hieult", mode
        )
    ...
    logger.info(
        f"Uploading document for project {workflow_id}: {filename} (Dify ID: {dify_document_id})"
    )
