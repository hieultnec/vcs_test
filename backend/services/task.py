...
def ex_task(t_task):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"First element of context: {info['context'][0] if info['context'] else 'Empty context'}")
                ...
                try:
                    ...
                    if retry_count > 0:
                        try:
                            ...
                            logger.warning(f"Failed to cleanup Chrome processes: {str(e)}")

                    remove_all_contents(f'projects/{info["project_id"]}/{info["task_id"]}')
                    ...
                    retry_count += 1
                    last_error = e
                    logger.warning(f"Task execution attempt {retry_count} failed: {str(e)}")
            ...
            # All retries failed
            logger.error(f"Task execution failed after {max_retries} attempts: {str(last_error)}")
