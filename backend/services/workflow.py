...
from services.dify_service import fetch_info, fetch_site, fetch_parameters, run_workflow_with_dify
...
def parse_user_input_form(user_input_form):
    ...
    for item in user_input_form:
        ...
        for extra in ["allowed_file_types", "allowed_file_upload_methods", "allowed_file_extensions", "max_length"]:
            if extra in field_def:
                input_obj[extra] = field_def[extra]
        inputs.append(input_obj)
    return inputs

def create_workflow(project_id, api_key):
    """
    Tạo workflow mới, chỉ cần project_id và api_key. Name/description lấy từ info/site nếu có.
    """
    workflow_id = str(uuid.uuid4())
    try:
        info = fetch_info(api_key)
        site = fetch_site(api_key)
        parameters = fetch_parameters(api_key)
        user_input_form = parameters.get("user_input_form", [])
        inputs = parse_user_input_form(user_input_form)
        wf_name = info.get("name") or site.get("title") or None
        wf_description = info.get("description") or site.get("description") or None
...
def upload_file_to_dify(workflow_id, file_storage):
    ...
    return upload_document(api_key, file_storage, workflow_id)
...
def run_dify_workflow(project_id, workflow_id, inputs, user="hieult", response_mode="blocking"):
    ...
    if not api_key:
        raise ValueError("API key not found in workflow")
    try:
        response_json = run_workflow_with_dify(api_key, inputs, user, response_mode)
